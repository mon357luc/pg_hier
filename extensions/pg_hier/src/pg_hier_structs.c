...
void
add_string_to_array(struct string_array *arr, char *value) {
    if ((arr->size + 1) >= arr->capacity) {
        arr->capacity = arr->capacity > 0 ? arr->capacity * 2 : 4;
        arr->data = arr->data ? 
            repalloc(arr->data, arr->capacity * sizeof(char *)) :
            palloc(arr->capacity * sizeof(char *));
    }
    arr->data[arr->size++] = pstrdup(value);
}

void
free_string_array(struct string_array *arr) {
    if (arr != NULL) {
        if (arr->data)
        {
...
void 
pop_table_stack(struct table_stack **stack) {
...
void
free_table_stack(struct table_stack *stack) {
    while (stack != NULL) {
